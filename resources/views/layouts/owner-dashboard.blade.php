        ...
        <div class="fixed inset-0 z-50 flex md:hidden" role="dialog" aria-modal="true" x-show="sidebarOpen" style="display: none;">
            ...
            <div class="relative flex w-full max-w-xs flex-1 flex-col bg-white pt-5 pb-4 shadow-xl"
                ...
                <div class="absolute top-0 right-0 -mr-12 pt-2">
                    <button type="button" class="ml-1 flex h-10 w-10 items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-white" @click="sidebarOpen = false">
                ...
                <div class="mt-8 h-0 flex-1 overflow-y-auto">
                    <nav class="space-y-1.5 px-3">
                        @include('owner.partials.sidebar-links')
                    </nav>
                </div>
            </div>
        </div>
            ...
            <header class="sticky top-0 z-10 flex h-16 flex-shrink-0 bg-white border-b border-slate-100 justify-between px-4 sm:px-6 lg:px-8 items-center shadow-sm">
                ...
                <div class="flex-1 flex items-center max-w-xs md:max-w-md">
                    <div class="relative w-full text-slate-400 focus-within:text-slate-600">
                        ...
                        <input type="text" placeholder="Search..." disabled class="block w-full pl-10 pr-3 py-2 border-transparent text-sm bg-slate-50 rounded-xl placeholder-slate-400 focus:outline-none cursor-not-allowed">
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative"
                         x-data="{
                             ...
                             fetchNotifications() {
                                 this.loading = true;
                                 fetch('{{ route('owner.notifications') }}', {
                             ...
                             statusColor(status) {
                                 const map = { pending: 'text-amber-600 bg-amber-50', confirmed: 'text-indigo-600 bg-indigo-50', completed: 'text-emerald-600 bg-emerald-50', cancelled: 'text-rose-600 bg-rose-50' };
                             ...
                             statusLabel(status) {
                                 const map = { pending: 'New Order Received', confirmed: 'Order Confirmed', completed: 'Order Completed', cancelled: 'Order Cancelled' };
                        ...
                        <div x-show="open"
                            ...
                            <div class="flex items-center justify-between px-4 py-3 border-b border-slate-50">
                                ...
                                <span class="text-xs text-slate-400 font-medium">New Orders</span>
                            </div>
                            ...
                            <div x-show="!loading" class="max-h-80 overflow-y-auto divide-y divide-slate-50">
                                <template x-if="notifications.length === 0">
                                    <div class="py-10 text-center px-4">
                                        ...
                                        <p class="text-sm font-semibold text-slate-500">No new orders</p>
                                        <p class="text-xs text-slate-400 mt-0.5">New customer orders will appear here</p>
                                    </div>
                                </template>

                                <template x-for="notif in notifications" :key="notif._id">
                                    <div class="flex items-start gap-3 px-4 py-3.5 hover:bg-slate-50 transition-colors">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-slate-500 mt-0.5">
                                                By <span class="font-medium text-slate-700" x-text="notif.user_name"></span>
                                            </p>
                    ...
                    <!-- Profile pill -->
                    <div class="flex items-center gap-2.5 bg-slate-50 border border-slate-200 rounded-xl pl-2 pr-3 py-1.5 cursor-default">
                        <div class="flex items-center justify-center w-7 h-7 rounded-lg bg-indigo-600 text-white font-bold text-xs">
                            {{ $initials }}
                        </div>
                        <div class="hidden sm:block">
                            <p class="text-xs font-bold text-slate-800 leading-none">{{ $user->name ?? 'Owner' }}</p>
                            <p class="text-[10px] text-slate-400 leading-none mt-0.5">Business Owner</p>
                        </div>
                        <svg class="w-3.5 h-3.5 text-slate-400 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
